...
exports.processTile = processTile;

function processTile(partial_report, callback)
{
    var projectId = partial_report.source_project_id;
    var zoom = partial_report.tile_z;
    var x = partial_report.tile_x;
    var y = partial_report.tile_y;
    ...
    // else...
    var tile_dir = config.tileRoot + "/" + projectId + "/" + zoom + "/" + y;
    var tile_path = tile_dir + "/" + x + ".jpg";

    // First create a structure to describe the requested tile
    var tile = new MapTile();
    tile.defineFromTileXY(Number(x), Number(y), Number(zoom));
    ...
    DataSource.GetPhotoForLocation(projectId, tc.lat, tc.lon, function(data){
        ...
        if (target_photo.id == partial_report.source_id)
        {
            console.log("Partial is best match");
            callback();
            return;
        } else {
            console.log("Found a better match! '"+target_photo.file+"' (was "+partial_report.source_file+")");
        }
        ...
        async.eachSeries(to_extract,
            function (tile_job, cb) {
                ...
                if (tile_job.tile.x == partial_report.tile_x &&
                        tile_job.tile.y == partial_report.tile_y &&
                        tile_job.tile.zoom == partial_report.tile_z)
                {
                    console.log("Extracting the single tile", x,y,zoom);
                } else {
                    cb();
                    return;
                }
